...
function update() {
  ...
  for (let s of [...stars]) {
    ...
    if (Math.abs(s.x - player.x) < player.size / 1.5 && Math.abs(s.y - player.y) < player.size / 1.5) {
      score += 10;
  ...
  for (let e of [...enemies]) {
    ...
    if (Math.abs(e.x - player.x) < player.size / 1.5 && Math.abs(e.y - player.y) < player.size / 1.5) {
      endGame();

...
const player = { x: 150, y: 500, size: 25, speed: 6, glow: 0 };
const stars = [];
const enemies = [];
const particles = [];
let score = 0;
...
const sfxLevelUp = new Audio("assets/levelup.mp3");

sfxStar.volume = 0.2; 
sfxGameOver.volume = 0.3; 
sfxLevelUp.volume = 0.2;
...
// Spawning
function spawnStar(){
  stars.push({ x: Math.random()*(canvas.width-15), y:0, size:10, flicker:Math.random(), sway:Math.random()*2-1 });
}
function spawnEnemy(){ enemies.push({ x: Math.random()*(canvas.width-25), y:0, size:20 }); }
function spawnParticles(x,y){
  for(let i=0;i<10;i++){
    particles.push({ x,y,vx:(Math.random()-0.5)*2,vy:-Math.random()*2-1,alpha:1,size:Math.random()*3+2 });
  }
}
...
function update(){
  ...
  for(let s of [...stars]){
    ...
    if(Math.abs(s.x-player.x)<player.size && Math.abs(s.y-player.y)<player.size){
      ...
      sfxStar.currentTime=0; sfxStar.play();
  ...
  for(let e of [...enemies]){
    ...
    if(Math.abs(e.x-player.x)<player.size && Math.abs(e.y-player.y)<player.size) endGame();
  ...
  if(score>=levelThreshold){
    ...
    sfxLevelUp.currentTime=0; sfxLevelUp.play();
  }
...
function endGame(){
  ...
  sfxGameOver.currentTime=0; sfxGameOver.play();
